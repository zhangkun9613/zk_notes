...
def crf_unary_score(tag_indices, sequence_lengths, inputs):
  ...
  if tag_indices.dtype == dtypes.int64:
    offsets = math_ops.cast(offsets, dtypes.int64) # offset为标签索引的位移量
  flattened_tag_indices = array_ops.reshape(offsets + tag_indices, [-1])

  unary_scores = array_ops.reshape(
      array_ops.gather(flattened_inputs, flattened_tag_indices), ## gather 取索引
      [batch_size, max_seq_len])

  masks = array_ops.sequence_mask(sequence_lengths,   # sequence_mask,因为mask_zero依然会保持计算
...
class CrfForowardRNNCell(RNNCell):
    def __init__(self,trans_param):
        self.n_labels = trans_param.shape.as_list()[1]
        self.trans_param = tf.expand_dims(trans_param,0) 
    ...
    def __call__(self,inputs,state,scope=None):
        """
        input batch_size * n_labels unary_score
        state batch_size * n_labels pre_cell output
        """
        state = tf.expand_dims(state,2) 
        trans_score = state + self.trans_param # batch_size * num_labels * num_labels
        output = inputs + tf.reduce_logsumexp(trans_score,axis=1) # Z_t+1^1 = \sigma{Z_t+1^i*G_i1}
        return output,output

class CRF(object): 
    def crf_look_likehood(self,inputs,labels,seq_lens):
            """
            input [batch_size * max_seqlen * num_labels] 在每个类别的概率,统一padding为max_seqlen ,补0
            labels [batch_size * max_seqlen * num_labels] one_hot 编码的label,便于计算路径分数
            seq_lens [batch_size]
            """
            num_labels = inputs.shape.as_list()[2]
            self.seq_lens = seq_lens - 1 # 第一个位置作为初始状态输入，因此序列长度减一
            self.trans_param = tf.Variable(name='trans_param',initial_value = tf.random_normal([num_labels,num_labels]))
            self.path_score = self.crf_path_score(inputs,labels,self.trans_param)
            self.log_norm = self.crf_log_norm(inputs,labels,self.trans_param,seq_lens)
            return self.log_norm - self.path_score
        
    def crf_path_score(self,inputs,labels,trans_param):
        # classification score
        unary_scores = tf.multiply(inputs,labels) # batch_size * max_seq_len * num_labels
        self.unary_scores = tf.reduce_sum(unary_scores,axis=[1, 2]) # batch_size,
        
        # transport score
        # 通过将索引转化为矩阵的方式来寻址，但是这样比较浪费空间，若label维度较大的时候还是用官方的实现方式吧
        label_l = tf.expand_dims(labels[:,0:-1],3)
        label_r = tf.expand_dims(labels[:,1:],2)
        y_pos = tf.matmul(label_l,label_r) # batch_size * max_seq_len-1 * num_labels * num_labels
        trans_expanded = tf.expand_dims(tf.expand_dims(trans_param,0),0) # 1 * 1 * num_labels * num_labels
        binary_score = tf.multiply(trans_expanded,y_pos)
        self.binary_score = tf.reduce_sum(binary_score,axis = [1,2,3])
        
        path_score = self.binary_score + self.unary_scores
        return tf.expand_dims(path_score,1)
    

    def crf_log_norm(self,inputs,labels,trans_param,seq_lens):
        init_state = inputs[:,0,:]
        rnn_inputs = inputs[:,1:,:]
        crfcell = CrfForowardRNNCell(trans_param)
        ouputs,state = tf.nn.dynamic_rnn(cell=crfcell,initial_state=init_state,sequence_length=seq_lens,inputs=rnn_inputs)
        log_norm = tf.reduce_logsumexp(state,1)
        return tf.expand_dims(log_norm,1)
                                    
                                        
crf = CRF()
inputs = tf.random_normal(shape= [10,15,20])
seq_length = 20*tf.ones(shape=[10])
labels = tf.random_normal(shape=[10,15,20])
loss = crf.crf_look_likehood(inputs,labels,seq_length)
