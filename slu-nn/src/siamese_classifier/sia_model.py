...
class SiameseLSTM(object):
  ...
  def __init__(self, max_len, embedding_size, hidden_units, l1_reg_lambda, batch_size, training):
    ...
    with tf.variable_scope("output") as scope:
      ...
      self.out2 = self.stackedRNN(input_x2, self.x2_seq_len, self.dropout_keep_prob, "side1",
                                  max_len, hidden_units)
      self.distance = tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(self.out1, self.out2)), 1, keep_dims=True))
      self.prob = tf.reshape(tf.sigmoid(self.distance), [-1], name = 'prob')
...
class LstmProb(object):

  def __init__(self, max_len, embedding_size, hidden_units, l1_reg_lambda, batch_size, training, num_classes = 2):
    self.input_x1_wv = tf.placeholder(tf.float32, [None, max_len, embedding_size], name="input_x1_wv")
    self.input_x1_pv = tf.placeholder(tf.float32, [None, max_len, 46], name="input_x1_pv")
    input_x1 = tf.concat([self.input_x1_wv, self.input_x1_pv], -1)
    self.x1_seq_len = tf.placeholder(tf.int32, [None], name="x1_seq_len")
    ...
    if training:
      ...
      self.dropout_keep_prob = tf.constant(1.0, tf.float32)
    with tf.variable_scope("output") as scope:
      self.out_vec = self.stackedRNN(input_x1, self.x1_seq_len, self.dropout_keep_prob, "side1",
                                  max_len, hidden_units)

      self.out_prob = tf.layers.dense(inputs=self.out_vec, units=num_classes)
      self.prob = tf.nn.softmax(self.out_prob)
    with tf.name_scope("loss"):
      self.loss = tf.losses.sparse_softmax_cross_entropy(logits=self.out_prob, labels=self.input_y)

    with tf.name_scope("accuracy"):
      self.predict = tf.argmax(self.out_prob, axis = 1)
